...
public:
    ...
    Q_INVOKABLE void setIsUser(bool x){
        isUser=x;
    }
    Q_INVOKABLE bool getIsUser(){
        return isUser;
    }
    ...
    Q_INVOKABLE bool authenticateApply(const QString& email){
        QSqlQuery query;
        ...
        if (!query.exec()) {
            qWarning() << "MyDatabase::authenticateApply - ERROR: " << query.lastError().text();
            return false;
        }
    ...
    Q_INVOKABLE void storeJobId(const QString& jobid){
        jobId=jobid;
    }
    ...
    Q_INVOKABLE QString getJobIdByEmployerId(const QString &employerId) {
        ...
        if (query.next()) {
            ...
            return query.value("job_id").toString();
        } else {
            qWarning() << "No record found for employer_id:" << employerId;
    ...
    Q_INVOKABLE bool registerUser(const QString& email, const QString& password, const QString& phint) {
        ...
        if(password==""){
            qWarning() << "MyDatabase::registerUser - ERROR: Fill Password";
            return false;
        }
    ...
    Q_INVOKABLE bool shortlistUser(const QString& userId){
        QSqlQuery query;
        query.prepare("UPDATE AppliedJobs SET status = :status WHERE user_id = :userId");
        query.bindValue(":status", "Shortlisted");
        query.bindValue(":userId", userId);
        query.exec();
        return true;
    }
    Q_INVOKABLE bool rejectUser(const QString& userId){
        QSqlQuery query;
        query.prepare("UPDATE AppliedJobs SET status = :status WHERE user_id = :userId");
    ...
    Q_INVOKABLE bool isKYCDataAvailable(const QString &email,int x) {
        ...
        if(x==0){
            QSqlQuery query;
            query.prepare("SELECT fullName AND address AND dobA AND dobB AND phone_number AND citizenshipf_link AND citizenshipb_link AND photo_link FROM Users WHERE email = ?");
...
int main(int argc, char *argv[])
{
    ...
    const QUrl url(u"qrc:/JobHub/Main.qml"_qs);
